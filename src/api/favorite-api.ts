...
class FavoriteApi {
  ...
  static async getFavorite(id: number): Promise<AlbumResponse[]> {
    try {
      const response = await this.axios.get<AlbumResponse[]>(
        `/favorite?userId=${id}`
      );
  ...
  static async verifyFavorite(
    userId: string,
    albumId: string
  ): Promise<boolean> {
    try {
      const response = await this.axios.get<boolean>(
        `/favorite/verify?userId=${userId}&albumId=${albumId}`
      );
  ...
  static async remove(payload: Favorite): Promise<void> {
    try {
      const response = await this.axios.delete<void>("/favorite", {
        data: payload,
      });
  ...
  static async removeFavoritesByAlbumId(id: number): Promise<void> {
    try {
      await this.axios.delete<void>(`/favorite/${id}`);
